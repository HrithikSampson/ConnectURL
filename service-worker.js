async function postData(url = "", data = {}) {
    ...
    // response.status; // parses JSON response into native JavaScript objects
  }
function processed_elements() {
  ...
  this.push = function (value) {
    ...
    if (this.processing_array.length == 5) {
      ...
      postData("http://localhost:3001/chrome-data", sendToAPI)
    }
  };
}
function processing_tabs() {
  ...
  this.add_first = function (details) {
    //add elements when the chrome reloads
    console.log(details);
  ...
  this.add = function (details) {
    ...
    this.processing_elements[details.tabId] = {
      ...this.processing_elements[details.tabId],
...
chrome.windows.getAll((window) => {
  chrome.tabs.query({ currentWindow: window.id }, (details) => {
    console.log("query");
    console.log(details);
    for (let detail of details) pt.add_first(detail);
  });
});
